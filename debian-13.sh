...
virt-customize -a "${IMAGE_NAME}" \
  ...
  \
  --update \
  --install "sudo,qemu-guest-agent,spice-vdagent,bash-completion,unzip,wget,curl,axel,net-tools,iputils-ping,iputils-arping,iputils-tracepath,vim,less,bzip2,mtr-tiny,btop,dnsutils,zstd,build-essential,git,tmux,zsh,lsof,ncat,locales,nfs-common,rsync,ncdu,iperf3,socat,fd-find,ripgrep,tree,ca-certificates,gnupg,lsb-release,bat,duf,zoxide"\
  \
